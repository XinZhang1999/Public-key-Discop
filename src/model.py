...
def get_model(settings: Settings) -> PreTrainedModel:
    if settings.task == 'text':
        if settings.model_name == 'gpt2-medium':
            model = GPT2LMHeadModel.from_pretrained("/home/goodbacteria/models/gpt2-medium").to(settings.device)
        elif settings.model_name == 'gpt2-large':
            model = GPT2LMHeadModel.from_pretrained("/home/goodbacteria/models/gpt2-large").to(settings.device)
            ...
            from transformers import AutoModelForCausalLM
            model = AutoModelForCausalLM.from_pretrained("/data/models/Llama-2-7b-hf").to(settings.device)
...
def get_tokenizer(settings: Settings) -> PreTrainedTokenizer:
    ...
    if settings.model_name == 'gpt2-medium':
        tokenizer = GPT2Tokenizer.from_pretrained("/home/goodbacteria/models/gpt2-medium")  # local_files_only=True
    elif settings.model_name == 'gpt2-large':
        tokenizer = GPT2Tokenizer.from_pretrained("/home/goodbacteria/models/gpt2-medium")  # local_files_only=True
    elif settings.model_name == 'baichuan':
